...
# === BALANCE DATASETS BY DOWNSAMPLING SQLi TO XSS SIZE ===
min_size = len(xss_df)
sqli_df_balanced = sqli_df.sample(n=min_size, random_state=42)
xss_df_balanced = xss_df.copy()
...
joblib.dump(vectorizer, os.path.join(MODEL_DIR, "MERGED_downsample_vectorizer.joblib"))
joblib.dump(clf, os.path.join(MODEL_DIR, "MERGED_downsample_model.joblib"))
